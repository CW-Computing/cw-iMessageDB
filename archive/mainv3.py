...
class IMessageDBApp:
    def __init__(self, root):
        ...
        self.chat_index_map = {}  # Add this line to store chat index mapping
        ...
        if not self.chats:  # Only create a new chat if no chats were loaded
            self.new_chat()
    ...
    def ask_question(self):
        ...
        # Rest of the existing ask_question code...
        try:
            import json
            if provider == "OpenAI":
                import openai
...
- message table: Contains messages with columns like text, handle_id (foreign key to handle.rowid), date, is_from_me
- handle table: Contains contact info with columns like id (phone number/email), rowid (primary key)
Only output the SQL query, no explanation. Return your answer as a JSON object with a single key 'sql', like this: {"sql": "SELECT ..."}"""}
                    ] + llm_messages,
                    max_tokens=500,
                    temperature=0,
...
- message table: Contains messages with columns like text, handle_id (foreign key to handle.rowid), date, is_from_me
- handle table: Contains contact info with columns like id (phone number/email), rowid (primary key)
To get message senders, you must JOIN handle ON message.handle_id = handle.rowid and use handle.id
Only output the SQL query, no explanation. Return your answer as a JSON object with a single key 'sql', like this: {"sql": "SELECT ..."}"""}
                    ] + llm_messages,
                    "max_tokens": 500,
                    "temperature": 0,
